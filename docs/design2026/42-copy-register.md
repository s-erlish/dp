...
kept; the PC keys are its 13.2 mapping.

**The connect object and its status line**

| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Disconnected | Отключено | Disconnected | `home_status_disconnected` | `Home_StatusDisconnected` | Главная shield, tray tooltip, quick tile | `AP` (R-20) |
...
| Disconnecting | Отключение… | Disconnecting… | `home_status_disconnecting` | `Home_StatusDisconnecting` | shield | `N` |
| Connect failed | Не удалось подключиться | Couldn't connect | `home_status_error` | `Home_StatusError` | shield (R-8) | `=` |
| No server chosen | Сервер не выбран | No server selected | `home_status_no_server` | `Home_StatusNoServer` | shield | `AP` |
| No servers at all | Нет серверов | No servers | `home_status_no_servers` | `Home_StatusNoServers` | shield | `=` |
| No subscription | Подписки нет | No subscription | `home_status_no_subscription` | `Home_StatusNoSubscription` | shield | `N` |
| Subscription expired | Подписка истекла | Subscription expired | `home_status_expired` | `Home_StatusExpired` | shield | `N` |
| Detail: retry hint | Нажмите, чтобы повторить | Tap to retry | `home_detail_retry` | `Home_DetailRetry` | under the shield, error state | `=` |
| Detail: pick a server | Выберите сервер в разделе «Серверы» | Choose a server on the Серверы tab | `home_detail_pick_server` | `Home_DetailPickServer` | under the shield, no-server state | `AP` |
| Connect (a11y) | Подключить | Connect | `home_cd_connect` | `Home_CdConnect` | shield `contentDescription`, tray, `depv://connect` | `A` |
| Disconnect (a11y) | Отключить | Disconnect | `home_cd_disconnect` | `Home_CdDisconnect` | shield, tray, `depv://disconnect` | `=` |
| Cancel connecting | Отменить подключение | Cancel connecting | `home_cd_cancel` | `Home_CdCancel` | shield while connecting | `N` |

**Notes.** `AP` on «Сервер не выбран»: Android ships «Выберите сервер» (`home_select_server`) and PC
ships the same as `Home_ChooseServer`; both are imperatives standing in a status slot, which reads
as an instruction where a state belongs. The instruction moves to the detail line, where it belongs.
...
| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Download column | Приём, Мбит/с | Down, Mbit/s | `home_strip_down_label` | `Home_StripDownLabel` | Главная strip | `N` |
| Upload column | Отдача, Мбит/с | Up, Mbit/s | `home_strip_up_label` | `Home_StripUpLabel` | Главная strip | `N` |
| Latency column | Задержка, мс | Latency, ms | `home_strip_ping_label` | `Home_StripPingLabel` | Главная strip | `N` |
| Session uptime | Время сессии | Session time | `home_strip_uptime_label` | `Home_StripUptimeLabel` | Главная strip, connected state | `N` |
...
| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Servers row | Серверы | Servers | `home_row_servers` | `Home_RowServers` | Главная ledger | `N` |
| Servers row value | %1$s · %2$s | %1$s · %2$s | `home_row_servers_value` | `Home_ServersProvidersMeta` | Главная ledger; assembled from two plurals (section 4) | `=` |
| Subscription row | Подписка | Subscription | `home_row_subscription` | `Home_RowSubscription` | Главная ledger | `N` |
| Active until | Активна до %1$s | Active until %1$s | `home_sub_active` | `Home_SubActive` | Главная ledger, Аккаунт card | `AP` (R-1) |
| Trial until | Пробный период до %1$s | Trial until %1$s | `home_sub_trial` | `Home_SubTrial` | Главная ledger | `N` |
| Expired on | Истекла %1$s | Expired %1$s | `home_sub_expired` | `Home_SubExpired` | Главная ledger | `N` |
| No subscription | Не оформлена | None | `home_sub_none` | `Home_SubNone` | Главная ledger | `N` |
| Could not refresh | Не удалось обновить | Couldn't refresh | `home_sub_stale` | `Home_SubStale` | Главная ledger | `N` |
| Expiring chip | Истекает | Expiring | `home_chip_expiring` | `Home_ChipExpiring` | Главная ledger, Аккаунт card | `=` |
| Expired chip | Истекла | Expired | `home_chip_expired` | `Home_ChipExpired` | Главная ledger, Аккаунт card, provider header | `P` |
| Account row title | Аккаунт | Account | `home_account_title` | `Home_AccountTitle` | Главная account chip | `=` |
| Account row subtitle | Вход, подписка, устройства | Sign-in, subscription, devices | `home_account_subtitle` | `Home_AccountSubtitle` | Главная account chip, signed out | `N` |
| Manage account | Управление аккаунтом | Manage account | `home_account_manage` | `Home_ManageAccount` | Главная account chip, signed in | `=` |

`P` on «Истекла»: PC declares it three times (`Account_HealthExpired`, `Account_ExpiredOn`,
`Sub_Expired`); two are deleted.

**The gate block** - what Главная offers when there is nothing to connect to

| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Sign-in caption | Войдите, чтобы получить серверы Departament. | Sign in to get Departament servers. | `home_gate_signin_caption` | `Home_GateSigninCaption` | Главная gate, signed out | `N` |
| Sign in | Войти | Sign in | `home_gate_signin` | `Home_GateSignin` | Главная gate | `=` |
| Add a provider | Добавить провайдера | Add a provider | `home_gate_add_provider` | `Common_AddSubscription` | Главная gate, Серверы empty state | `=` |
| Buy caption | Осталось выбрать тариф. | One step left: choose a plan. | `home_gate_buy_caption` | `Home_GateBuyCaption` | Главная gate, signed in, no subscription | `N` |
| Buy | Купить подписку | Buy a subscription | `home_gate_buy` | `Common_BuySubscription` | Главная gate, Аккаунт row, Купить title | `=` |
| Sync caption | Подписка активна, серверы ещё не загружены. | Your subscription is active, the servers aren't loaded yet. | `home_gate_sync_caption` | `Home_GateSyncCaption` | Главная gate | `N` |
| Load servers | Загрузить серверы | Load servers | `home_gate_sync` | `Home_GateSync` | Главная gate | `N` |

**The status strip conditions** (`11-app-structure.md` 8.2; one strip at a time, in this priority)

| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Offline | Нет сети. Показаны последние данные. | You're offline. Showing the last known data. | `strip_offline` | `Strip_Offline` | Главная, Серверы, Аккаунт; action «Повторить» | `N` |
| Stale data note | Данные могли устареть | This data may be out of date | `strip_stale` | `Strip_Stale` | under a stale value on any of the three | `N` |
| Subscription expired | Подписка истекла. Продлите её, чтобы подключаться. | Your subscription has expired. Renew it to connect. | `strip_expired` | `Strip_Expired` | Главная, Серверы, Аккаунт; action «Продлить» | `A+` |
| Expiring soon | Подписка заканчивается %1$s. | Your subscription ends on %1$s. | `strip_expiring` | `Strip_Expiring` | Главная; action «Продлить» | `N` |
| Device limit | Достигнут лимит устройств. | You've reached the device limit. | `strip_devices` | `Strip_Devices` | Главная, Аккаунт; action «Устройства» | `A+` |
| Provider refresh failed | Не удалось обновить провайдера. | Couldn't refresh the provider. | `strip_provider_failed` | `Strip_ProviderFailed` | Серверы; action «Повторить» | `N` |
| Server went silent | Сервер не отвечает. Выберите другой сервер. | The server isn't responding. Choose another one. | `strip_silent` | `Strip_Silent` | Главная; action «Сменить сервер» | `N` |
| TUN needs rights | Режим VPN недоступен без прав администратора | VPN mode needs administrator rights | - | `Home_TunUnavailable` | PC Главная; action «Перезапустить с правами» | `P` (R-12) |
| Restart elevated | Перезапустить с правами | Restart as administrator | - | `Home_RestartElevated` | PC Главная | `=` |
| Strip actions | Продлить · Повторить · Устройства · Сменить сервер · Как исправить | Renew · Try again · Devices · Change server · How to fix | `home_action_renew`, `common_retry`, `home_action_devices`, `home_action_change_server`, `home_action_howto` | `Home_ActionRenew`, `Common_Retry`, `Home_ActionDevices`, `Home_ActionChangeServer`, `Home_ActionHowto` | as above | `N` |
...
`home_select_server`, `speed_zero`, `connection_connected` («Соединено, нажмите для проверки»),
`connection_not_connected`, `memory_app_usage`, `memory_normal/elevated/high`, `memory_value`
(`%1$d MB · %2$s`), `sub_days_left` (`%1$s · %2$dd`).
On PC: `Home_Welcome`, `Home_NoSubs`, `Home_NoSubsHint`, `Home_ChooseServer`, `Home_NotConnected`,
`Home_MyServers`, `Status_Disconnected`, `Status_ConnectedTo`, `Onboarding_Title`,
`Onboarding_Subtitle`, `Onboarding_OrSignIn`, `Onboarding_OrSignInShort` (the onboarding view is
deleted, `14-auth.md` PG-A3).
...
| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Screen title | Серверы | Servers | `servers_title` | `Servers_Title` | Серверы header; PC list band header | `AP` |
| Count line | %1$s · %2$s | %1$s · %2$s | `servers_count` | `Servers_Count` | header meta; two plurals (section 4) | `N` |
| Sort | Сортировка | Sort | `servers_sort_title` | `Servers_SortTitle` | header overflow | `N` |
| Sort: provider order | Как у провайдера | Provider order | `servers_sort_provider` | `Servers_SortProvider` | sort sheet, Настройки → Провайдеры | `N` |
| Sort: latency | По задержке | By latency | `servers_sort_ping` | `Servers_SortPing` | sort sheet | `N` |
| Sort: name | По названию | By name | `servers_sort_name` | `Servers_SortName` | sort sheet | `N` |
| Sort: manual | Вручную | Manual | `servers_sort_manual` | `Servers_SortManual` | sort sheet | `N` |
| More (a11y) | Ещё | More | `servers_more_cd` | `Servers_MoreCd` | header overflow | `N` |
| Provider actions (a11y) | Действия провайдера | Provider actions | `servers_group_actions_cd` | `Servers_GroupActionsCd` | group header kebab | `N` |

**Group headers and their state line**

| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Manually added group | Добавленные вручную | Added manually | `servers_group_local` | `Servers_GroupLocal` | group header | `AP` |
| Not refreshed | Не обновился | Not refreshed | `servers_group_stale` | `Servers_GroupStale` | group header | `N` |
| Data may be stale | Данные могли устареть | This data may be out of date | `servers_group_offline` | `Servers_GroupOffline` | group header, offline | `N` |
| Refreshing | Обновляется… | Refreshing… | `servers_group_updating` | `Servers_GroupUpdating` | group header | `N` |
| Search result count | Найдено: %1$d из %2$d | Found %1$d of %2$d | `servers_group_found` | `Servers_GroupFound` | header meta while filtering | `N` |
| Auto-update value | Автообновление · %1$s | Auto-update · %1$s | `servers_provider_auto` | `Sub_AutoUpdate` | provider header | `A` |

`AP` on «Добавленные вручную»: Android ships «Локальные», which is developer vocabulary.

**The server row**

| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Latency value | %1$d мс | %1$d ms | `servers_ping_unit` | `Servers_PingUnit` | server row trailing | `AP` |
| No answer | нет ответа | no answer | `servers_ping_none` | `Servers_PingNone` | server row trailing | `N` |
| Connected state | Подключено | Connected | `home_status_connected` | `Home_StatusConnected` | server row, shared key | `=` |
| Row (a11y) | %1$s, %2$s | %1$s, %2$s | `servers_row_cd` | `Servers_RowCd` | TalkBack / Narrator: name, then state | `N` |
| Long-press hint | Удерживайте сервер, чтобы открыть действия | Press and hold a server to open its actions | `servers_longpress_hint` | - | first run on Серверы (Android) | `N` |

`AP` on the latency unit: Android formats it in Kotlin
(`dto/entities/ServerAffiliationInfo.kt:76` holds the literal «мс»), PC formats it in the view. Both
...
| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Connect | Подключиться | Connect | `servers_action_connect` | `Servers_ActionConnect` | actions sheet | `N` |
| Make default | Сделать основным | Make default | `servers_action_set_default` | `Servers_MakeDefault` | actions sheet | `=` |
| Test latency | Проверить задержку | Test latency | `servers_action_ping` | `Common_TestLatency` | actions sheet, header overflow | `A` |
| Edit | Изменить | Edit | `servers_action_edit` | `Common_Edit` | actions sheet | `=` |
| Duplicate | Дублировать | Duplicate | `servers_action_duplicate` | `Servers_Duplicate` | actions sheet | `=` |
| Duplicate suffix | (копия) | (copy) | `servers_action_duplicate_suffix` | `Servers_DuplicateSuffix` | appended to the new name | `P+` |
| Move | Переместить | Move | `servers_action_move` | `Servers_ActionMove` | actions sheet → sub-sheet | `N` |
| Move to top | В начало | To the top | `servers_action_move_top` | `Servers_ActionMoveTop` | move sub-sheet | `N` |
| Move up | Выше | Up | `servers_action_move_up` | `Servers_ActionMoveUp` | move sub-sheet | `N` |
| Move down | Ниже | Down | `servers_action_move_down` | `Servers_ActionMoveDown` | move sub-sheet | `N` |
| Move to bottom | В конец | To the bottom | `servers_action_move_bottom` | `Servers_ActionMoveBottom` | move sub-sheet | `N` |
| Move to another group | В другую группу… | To another group… | `servers_action_move_group` | `Servers_ActionMoveGroup` | move sub-sheet | `N` |
| Position | Позиция %1$d из %2$d | Position %1$d of %2$d | `servers_action_position` | `Servers_ActionPosition` | move sub-sheet header | `N` |
| Copy the link | Скопировать ссылку | Copy link | `servers_action_copy_link` | `Servers_ActionCopyLink` | actions sheet | `AP` |
| Share a QR code | Поделиться QR-кодом | Share QR code | `servers_action_share_qr` | `Servers_ActionShareQr` | actions sheet | `AP` |
| Delete the server | Удалить сервер | Delete server | `servers_action_delete` | `Servers_ActionDelete` | actions sheet, destructive | `AP` |

`AP` on the three share/delete rows: Android ships «Поделиться (QR)» / «Поделиться (буфер)» /
...
and drops the parenthetical.

**Provider actions** (the group header kebab)

| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Refresh | Обновить | Refresh | `servers_provider_refresh` | `Common_UpdateSubscription` | provider kebab | `AP` |
| Rename | Переименовать | Rename | `servers_provider_rename` | `Servers_ProviderRename` | provider kebab | `N` |
| Pin | Закрепить | Pin | `servers_provider_pin` | `Sub_Pin` | provider kebab | `A` |
| Unpin | Открепить | Unpin | `servers_provider_unpin` | `Servers_ProviderUnpin` | provider kebab | `A` |
| Open the link | Открыть ссылку | Open link | `servers_provider_open_link` | `Servers_ProviderOpenLink` | provider kebab | `N` |
| Provider settings | Настройки провайдера | Provider settings | `servers_provider_settings` | `Servers_ProviderSettings` | provider kebab | `N` |
| Delete the provider | Удалить провайдера | Delete provider | `servers_provider_delete` | `Sub_Delete` | provider kebab, destructive | `A` |
| Support | Поддержка | Support | - | `Sub_Support` | PC provider header | `=` |
| Open support | Открыть поддержку | Open support | - | `Sub_OpenSupport` | PC provider header | `=` |

`A` on the four provider rows: Android says «подписка» for all of them (`sub_delete`, `sub_pin`,
`sub_unpin`, `title_sub_update`), which is the single most widespread terminology-lock break in the
product - 48 live Android strings contain the root «подпис» and about half of them mean provider
(`40-copy-inventory-android.md` F5.1). PC already says «провайдер» throughout.

**Adding servers**

| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Add sheet title | Добавить | Add | `servers_add_title` | `Servers_AddTitle` | add sheet | `=` |
| Add a provider | Добавить провайдера | Add a provider | `servers_add_provider` | `Common_AddSubscription` | add sheet, empty state, Главная gate | `=` |
| Scan a QR code | Сканировать QR-код | Scan a QR code | `servers_add_qr` | `Common_AddViaQr` | add sheet | `A` |
| Paste from the clipboard | Вставить из буфера | Paste from clipboard | `servers_add_clipboard` | `Common_AddFromClipboard` | add sheet | `AP` |
| Enter a link | Ввести ссылку | Enter a link | `servers_add_link` | `Servers_AddLink` | add sheet | `P+` |
| Import from a file | Импортировать из файла | Import from a file | `servers_add_file` | `Servers_AddFile` | add sheet | `P+` |
| Create manually | Создать вручную | Create manually | `servers_add_manual` | `Servers_AddManual` | add sheet | `P+` |
| Link field label | Ссылка провайдера или сервера | Provider or server link | `servers_add_link_label` | `Servers_AddLinkLabel` | manual-entry sheet | `A+` |
| Link field hint | Вставьте ссылку из бота или адрес сервера | Paste the link from the bot or a server address | `servers_add_link_hint` | `Servers_AddLinkHint` | manual-entry sheet | `A+` |

`A+` on the two field strings: `MainActivity.kt:2497-2500` hardcodes both, and the second leaks a
fake domain (`https://departament.example/sub`) into user-visible copy.

**Bulk actions in the header overflow**

| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Update providers | Обновить провайдеров | Refresh providers | `servers_menu_update` | `Servers_MenuUpdate` | header overflow | `AP` |
| Test latency, scoped | Проверить задержку: %1$s | Test latency: %1$s | `servers_menu_ping_scoped` | `Servers_MenuPingScoped` | header overflow, names the scope | `N` |
| Stop testing | Остановить проверку | Stop testing | `servers_menu_ping_stop` | `Servers_MenuPingStop` | header overflow while testing | `N` |
| Collapse all | Свернуть все группы | Collapse all groups | `servers_menu_collapse` | `Servers_MenuCollapse` | header overflow | `N` |
| Expand all | Развернуть все группы | Expand all groups | `servers_menu_expand` | `Servers_MenuExpand` | header overflow | `N` |
| Export to the clipboard | Экспортировать в буфер | Export to clipboard | `servers_menu_export` | `Servers_MenuExport` | header overflow | `P+` |
| Delete duplicates | Удалить дубликаты | Delete duplicates | `servers_menu_del_duplicate` | `Servers_MenuDelDuplicate` | header overflow | `P+` |
| Delete unreachable | Удалить недоступные | Delete unreachable | `servers_menu_del_invalid` | `Servers_MenuDelInvalid` | header overflow | `P+` |
| Delete all servers | Удалить все серверы | Delete all servers | `servers_menu_del_all` | `Servers_MenuDelAll` | header overflow, destructive | `P+` |
| Selected count (PC) | Выбрано: %1$d | %1$d selected | - | `Servers_SelectedCount` | PC multi-select (PG-1) | `P+` |
| Clear selection (PC) | Снять выделение | Clear selection | - | `Servers_ClearSelection` | PC multi-select | `P+` |

**Transient confirmations**

| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Server deleted | Сервер удалён | Server deleted | `servers_deleted` | `Servers_Deleted` | snackbar / toast with «Отменить» | `N` |
| Provider deleted | Провайдер удалён | Provider deleted | `servers_provider_deleted` | `Servers_ProviderDeleted` | snackbar with «Отменить» | `N` |
| Link copied | Ссылка скопирована | Link copied | `servers_link_copied` | `Servers_LinkCopied` | snackbar | `N` |
| Refreshed | Обновлено: %1$s | Refreshed: %1$s | `servers_refreshed` | `Servers_Refreshed` | snackbar after a provider refresh | `N` |
| Servers added | Добавлено: %1$s | Added: %1$s | `servers_added` | `Servers_Added` | snackbar after an import; %1$s is a plural | `A+` |
| Provider already added | Провайдер уже добавлен | This provider is already added | `servers_provider_exists` | `Servers_ProviderExists` | snackbar after a duplicate import | `A+` |
| Not our link | Эта ссылка не от departament. Используйте ссылку из бота. | This link isn't from departament. Use the link from the bot. | `servers_foreign_link` | `Servers_ForeignLink` | snackbar after an import | `A+` |

`A+` on the last three: all three are hardcoded Kotlin literals today, the last one in **three**
...
| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Tab title | Аккаунт | Account | `account_tab_title` | `Account_Title` | Аккаунт header | `=` |
| Name fallback | Аккаунт | Account | `account_name_fallback` | `Account_NameFallback` | head, when no name is known | `=` |
| Telegram not linked | Telegram не привязан | Telegram isn't linked | `account_no_telegram` | `Account_NoTelegram` | head handle slot, sign-in methods | `P+` |
| Change photo | Сменить фото | Change photo | `account_change_avatar` | `Account_ChangeAvatar` | head avatar sheet | `P+` |
| Pick from the gallery | Выбрать из галереи | Choose from gallery | `account_avatar_gallery` | `Account_AvatarGallery` | avatar sheet | `P+` |
| Remove the photo | Убрать фото | Remove photo | `account_avatar_remove` | `Account_AvatarRemove` | avatar sheet | `P+` |
| Avatar updated | Аватар обновлён | Photo updated | `account_avatar_updated` | `Account_AvatarUpdated` | transient | `P+` |
| Avatar failed | Не удалось загрузить фото. Выберите другое изображение. | Couldn't upload the photo. Choose another image. | `account_avatar_error` | `Account_AvatarError` | transient with «Повторить» | `AP` |
| Gate title | Вход в departament | Sign in to departament | `account_gate_title` | `Account_GateTitle` | Аккаунт signed out; identical to the auth gate | `=` |
| Gate body | Здесь будут подписка, устройства и платежи. | Your subscription, devices and payments live here. | `account_gate_body` | `Account_GateBody` | Аккаунт signed out | `P` |
| Gate: Telegram | Войти через Telegram | Sign in with Telegram | `account_gate_telegram` | `Common_SignInTelegram` | gate, auth surface A | `=` |
| Gate: email | Войти по почте | Sign in with email | `account_gate_email` | `Account_GateEmail` | gate, auth surface A | `P` |

`AP` on the avatar failure: neither platform completes 9.4's formula today (Android stops at «Не
удалось загрузить фото»). `P` on the gate body: PC's `Account_SignInHint` («Через Telegram быстро и
без пароля. Или войдите по почте на сайте.») describes methods the buttons already name, and it says
«на сайте», which is no longer true - registration happens in the app.

**3.4.2 Subscription switcher and card**

| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Switcher label | Подписка | Subscription | `account_switcher_title` | `Account_SwitcherTitle` | above the card | `=` |
| Switcher sheet title | Выберите подписку | Choose a subscription | `account_switcher_sheet_title` | `Account_SwitcherSheetTitle` | switcher sheet | `P+` |
| Unnamed subscription | Подписка %1$d | Subscription %1$d | `account_sub_default_name` | `Account_SubscriptionN` | switcher, card title | `=` |
| Perpetual title | Бессрочная подписка | No expiry | `account_card_perpetual_title` | `Account_PerpetualTitle` | card, perpetual | `P` |
| Perpetual detail | Срок не ограничен | There's no end date | `account_card_perpetual_detail` | `Account_PerpetualDetail` | card, perpetual | `P+` |
| Active until | Активна до %1$s | Active until %1$s | `account_card_active_title` | `Account_ActiveTitle` | card, Главная ledger | `AP` (R-1) |
| Remaining | Осталось %1$s | %1$s left | `account_card_active_detail` | `Account_ActiveDetail` | card, 8-30 days; %1$s is a plural | `AP` |
| Expiring today | Истекает сегодня | Expires today | `account_card_today_title` | `Account_TodayTitle` | card, last day | `N` |
| Last day detail | Сегодня последний день | Today is the last day | `account_card_today_detail` | `Account_TodayDetail` | card, last day | `N` |
| Expired title | Подписка истекла | Subscription expired | `account_card_expired_title` | `Account_ExpiredTitle` | card, expired | `P` |
| Expired detail | Срок закончился %1$s | It ended on %1$s | `account_card_expired_detail` | `Account_ExpiredDetail` | card, expired | `N` |
| Unknown expiry | Срок неизвестен | End date unknown | `account_card_unknown_title` | `Account_UnknownTitle` | card, no date from the backend | `N` |
| Unknown detail | Обновите страницу или проверьте позже | Refresh, or check again later | `account_card_unknown_detail` | `Account_UnknownDetail` | card | `N` |
| Health: active | Активна | Active | `account_health_active` | `Account_HealthActive` | card chip | `=` |
| Health: expiring | Истекает | Expiring | `account_health_expiring` | `Account_HealthExpiring` | card chip, Главная ledger | `=` |
| Health: expired | Истекла | Expired | `account_health_expired` | `Account_HealthExpired` | card chip, Главная ledger, provider header | `=` |
| Trial chip | Пробный | Trial | `account_card_trial_badge` | `Account_TrialBadge` | card chip | `AP` (R-6) |
| Plan caption | Тариф · %1$s | Plan · %1$s | `account_tariff_caption` | `Account_TariffCaption` | card, under the name | `=` |
| Traffic label | Трафик | Traffic | `account_card_traffic_label` | `Account_TrafficLabel` | card meter | `P+` |
| Traffic value | %1$s из %2$s | %1$s of %2$s | `account_card_traffic_value` | `Account_TrafficValue` | card meter | `AP` |
| Traffic exhausted | Трафик исчерпан | You're out of traffic | `account_card_traffic_over` | `Account_TrafficOver` | card meter at 100 % | `N` |
| Renew | Продлить | Renew | `account_card_renew` | `Account_Renew` | card CTA, strip action | `=` |
| Renew with a price | Продлить · %1$s | Renew · %1$s | `account_card_renew_price` | `Account_RenewPrice` | card CTA when a price is known | `P+` |
| Buy | Купить | Buy | `account_card_buy` | `Account_PickPlan` | card CTA, empty state | `=` (R-5) |
| Upgrade the plan | Улучшить тариф | Upgrade plan | `account_card_upgrade` | `Account_UpgradeTariff` | card overflow, upgrade sheet title | `=` |
| Rename the subscription | Переименовать подписку | Rename subscription | `account_card_rename` | `Account_Rename` | card overflow | `P+` |
| Previous / next | Предыдущая · Следующая | Previous · Next | `account_switch_prev`, `account_switch_next` | `Account_PrevSub`, `Account_NextSub` | switcher arrows (a11y) | `A+` |

**Notes.** `AP` on «Активна до»: PC ships four competing expiry formats and Android ships «Действует
до %1$s» (`account_expires`); R-1 settles it. `AP` on «Осталось %1$s»: PC ships
`Account_ExpiresInDays` = «Осталось {0} дн.» - an abbreviation where 9.2 wants a word, and no plural
(section 4). `AP` on the traffic value: PC ships `Account_TrafficUnlimited` («{0} · безлимит») and
Android ships `sub_traffic_used` («%1$s / %2$s»); the unit appears once, at the end, and an
unlimited plan renders **no meter at all** rather than a «безлимит» line
(`23-account-rework.md` 4.5). Deleted with the card: `Account_ValidUntil`, `Account_ExpiresUntil`,
`Account_ExpiredOn`, `Account_ExpiresInDays`, `Account_Perpetual`, `Sub_Until`,
`account_trial_badge` («ПРОБНЫЙ», the product's only ALL-CAPS Cyrillic string), `account_card_buy_tariff`,
`account_card_pick_tariff`, `sub_infinity`, `account_unlimited`, `buy_unlimited`.

**3.4.3 Auto-renew**

| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Row label | Автопродление | Auto-renew | `account_auto_renew` | `Account_AutoRenew` | card row | `P` |
| Next charge | %1$s спишем %2$s | On %1$s we'll charge %2$s | `account_auto_renew_next` | `Account_AutoRenewNext` | row subtitle, on | `AP` |
| On | Продлим автоматически | We'll renew it for you | `account_auto_renew_on` | `Account_AutoRenewOn` | row subtitle, on, no price known | `AP` |
| Off | Продление вручную | You renew it yourself | `account_auto_renew_off` | `Account_AutoRenewOff` | row subtitle, off | `AP` |
| Risk line | Без автопродления доступ прервётся %1$s | Without auto-renew your access stops on %1$s | `account_auto_renew_risk` | `Account_AutoRenewRisk` | row subtitle, off and expiring | `AP` |
| Saving | Сохраняем… | Saving… | `account_auto_renew_saving` | `Account_AutoRenewSaving` | row trailing, in flight | `N` |
| Failed | Не удалось изменить автопродление. Повторите. | Couldn't change auto-renew. Try again. | `account_auto_renew_failed` | `Account_AutoRenewFailed` | inline under the row, with «Повторить» | `N` |

`AP` throughout: PC's current set states the switch's own position back to the user
(«Автопродление включено» / «Автопродление выключено»), which is what the switch already shows. The
new set states the **consequence**, which is the only thing a subtitle is for
(`12-settings.md` 11.6.1). `P` on the label: `Account_AutoRenew` exists on PC and is never rendered.

**3.4.4 Groups and rows**

| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Group: subscription | Подписка | Subscription | `account_group_subscription` | `Account_GroupSubscription` | Аккаунт | `N` |
| Group: billing | Оплата | Billing | `account_group_billing` | `Account_GroupBilling` | Аккаунт | `N` |
| Group: sign-in | Вход | Sign-in | `account_group_signin` | `Account_GroupSignIn` | Аккаунт | `N` |
| Devices row | Устройства | Devices | `account_row_devices` | `Account_Devices` | Аккаунт row, Устройства title, strip action | `=` |
| Device pair | %1$d / %2$d | %1$d / %2$d | `account_devices_pair` | `Account_DevicesPair` | devices row value | `AP` |
| Unlimited devices | Без ограничений | Unlimited | `account_devices_unlimited` | `Account_DevicesUnlimited` | devices row subtitle | `AP` |
| Slots on this subscription | Слотов на подписке | Slots on this subscription | `account_devices_slots` | `Account_DevicesSlots` | devices row subtitle, secondary sub | `N` |
...
| Buy a subscription | Купить подписку | Buy a subscription | `account_row_buy` | `Common_BuySubscription` | Аккаунт row, Купить title, Главная gate | `=` |
| Payment history | История платежей | Payment history | `account_row_history` | `Common_PaymentHistory` | Аккаунт row, sub-page title | `=` |
| Referral code | Реферальный код | Referral code | `account_row_referral` | `Account_ReferralCode` | Аккаунт row | `AP` |
| Copy the code | Скопировать код | Copy code | `account_copy_referral` | `Account_CopyReferralCode` | referral row action | `=` |
| Code copied | Код скопирован | Code copied | `account_referral_copied` | `Account_ReferralCopied` | transient | `A+` |
| Sign-in methods | Способы входа | Sign-in methods | `account_row_login_methods` | `Account_LinkingTitle` | Аккаунт row, sub-page title | `=` |
| Link Telegram | Привязать Telegram | Link Telegram | `account_row_link_telegram` | `Account_RowLinkTelegram` | Аккаунт row, sign-in methods, empty state action | `P+` |
| Link Telegram subtitle | Управление подпиской из бота | Manage your subscription from the bot | `account_row_link_telegram_sub` | `Account_RowLinkTelegramSub` | that row | `P+` |
| Sign out | Выйти | Sign out | `account_row_logout` | `Account_SignOut` | Аккаунт row, destructive | `=` |

`AP` on the referral row: Android ships «Реф-код %1$s» and PC ships «Реф-код {0}» - an abbreviation
inside a label, and the code belongs in the row's value slot, not inside its title. `AP` on the
device pair and «Без ограничений»: PC ships four device-count formats
(`Account_DevicesCount`, `Account_DevicesShort`, `Account_DevicesUsage`, `Account_DevicesTotal`) and
Android ships two; one pair format and one unlimited word survive. `∞` is not used: it is not
guaranteed in the vendored font (`23-account-rework.md` 4.5), which also deletes `sub_infinity`,
`account_unlimited` and `buy_unlimited`. `P+` on «Привязать Telegram»: PC has **no such string at
all**, only `Account_LinkAction` = «Привязать», although 9.3 locks the phrase and 0.4.9 makes the
CTA an explicit owner request.

**3.4.5 Payment and top-up sheet**

| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Sheet title | Оплата | Payment | `pay_sheet_title` | `Pay_SheetTitle` | pay sheet | `N` |
| Total | Итого | Total | `pay_total` | `Buy_Total` | pay sheet, Купить | `=` |
| Approximate | Примерно %1$s | About %1$s | `pay_estimate` | `Pay_Estimate` | pay sheet, add-devices | `AP` |
| Estimate note | Точную сумму покажем при оплате | We'll show the exact amount at checkout | `pay_estimate_note` | `Pay_EstimateNote` | pay sheet | `AP` |
| Pay from the balance | Оплатить с баланса | Pay from balance | `pay_from_balance` | `Pay_FromBalance` | pay sheet | `AP` |
| Balance available | На балансе %1$s | %1$s on your balance | `pay_balance_have` | `Pay_BalanceHave` | pay sheet | `N` |
| Balance short | Не хватает %1$s | %1$s short | `pay_balance_short` | `Pay_BalanceShort` | pay sheet | `N` |
| Pay by card | Оплатить картой | Pay by card | `pay_by_card` | `Pay_ByCard` | pay sheet | `P` |
| Pay via SBP | Оплатить через СБП | Pay via SBP | `pay_by_sbp` | `Pay_BySbp` | pay sheet | `N` |
| Pay via other | Оплатить через %1$s | Pay via %1$s | `pay_by_other` | `Pay_ByOther` | pay sheet | `N` |
| Payment method | Способ оплаты | Payment method | `pay_method_title` | `Buy_PaymentMethod` | pay sheet header | `=` |
| Subject: renewal | Продление %1$s, %2$s | Renewing %1$s, %2$s | `pay_subject_renew` | `Pay_SubjectRenew` | pay sheet subject line | `N` |
| Subject: upgrade | Улучшение до %1$s, +%2$s | Upgrade to %1$s, +%2$s | `pay_subject_upgrade` | `Pay_SubjectUpgrade` | pay sheet subject line | `N` |
| Subject: devices | %1$s к подписке «%2$s» | %1$s for subscription "%2$s" | `pay_subject_devices` | `Pay_SubjectDevices` | pay sheet subject line | `N` |
| Subject: top-up | Пополнение баланса | Balance top-up | `pay_subject_topup` | `Account_TopUpTitle` | pay sheet, history row kind | `=` |
| Top-up title | Пополнение баланса | Top up balance | `topup_title` | `Account_TopUpTitle` | top-up sheet | `A` |
| Amount label | Сумма | Amount | `topup_amount_label` | `TopUp_AmountLabel` | top-up field label (C7) | `AP` |
| Amount empty | Введите сумму | Enter an amount | `topup_error_empty` | `TopUp_ErrorEmpty` | field error | `A` |
| Amount zero | Сумма должна быть больше 0 | The amount must be greater than 0 | `topup_error_zero` | `TopUp_ErrorZero` | field error | `AP` |
| Amount not a number | Введите сумму цифрами | Enter the amount in digits | `topup_error_format` | `TopUp_ErrorFormat` | field error | `A` |
| Continue | Продолжить | Continue | `pay_continue` | `Account_Continue` | top-up sheet | `A+` |
| Upgrade sheet title | Улучшить тариф | Upgrade plan | `upgrade_sheet_title` | `Upgrade_SheetTitle` | upgrade sheet | `=` |
| Upgrade note | Доплата рассчитывается за оставшийся срок | The difference is prorated over the time you have left | `upgrade_sheet_note` | `Upgrade_SheetNote` | upgrade sheet | `N` |
| Upgrade quote | %1$s · +%2$s | %1$s · +%2$s | `upgrade_quote` | `Account_UpgradeQuote` | upgrade option row; %2$s is a plural | `A+` |
| No upgrades left | Вы на максимальном тарифе | You're on the top plan | `upgrade_none` | `Account_NoUpgrades` | upgrade sheet, empty | `A+` |
| Rename title | Название подписки | Subscription name | `rename_title` | `Rename_Title` | rename sheet | `N` |
| Rename label | Название | Name | `rename_label` | `Rename_Label` | rename field | `N` |
| Rename empty | Введите название | Enter a name | `rename_error_empty` | `Rename_ErrorEmpty` | field error | `N` |
| Rename failed | Не удалось переименовать. Повторите. | Couldn't rename it. Try again. | `rename_failed` | `Rename_Failed` | inline, with «Повторить» | `N` |
...
| Checking the payment | Проверяем оплату… | Checking the payment… | `account_pay_checking` | `Account_PayChecking` | Аккаунт strip while polling | `N` |
| Payment done | Оплата прошла | Payment complete | `account_pay_done` | `Account_PayDone` | transient | `N` |
| Payment unconfirmed | Не удалось подтвердить оплату. Проверьте историю платежей. | We couldn't confirm the payment. Check your payment history. | `account_pay_unconfirmed` | `Account_PayUnconfirmed` | strip with «История» | `N` |
| Open the history | История | History | `account_pay_open_history` | `Account_PayOpenHistory` | that strip's action | `N` |

`AP` on the estimate pair: PC's `Account_DeviceEstimate` («≈ {0}») uses a glyph where a word belongs
and `Account_EstimateNote` («Примерная сумма, точную посчитаем при оплате») restates itself. `A` on
the top-up strings: Android's «Сумма пополнения» / «Введите сумму» / «Введите корректную сумму» -
«корректную» tells the user nothing about what is wrong.

**3.4.6 Devices**

| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Screen title | Устройства | Devices | `devices_title` | `Devices_Title` | sub-page | `=` |
| Subtitle | Устройства, подключённые к вашей подписке | Devices connected to your subscription | `devices_subtitle` | `Devices_Subtitle` | sub-page intro | `=` |
| Connect a device | Подключить устройство | Connect a device | `devices_connect_header` | `Devices_ConnectHeader` | section header | `N` |
| Show a QR code | Показать QR-код | Show QR code | `devices_row_qr` | `Devices_RowQr` | row | `N` |
| Copy the link | Скопировать ссылку | Copy link | `devices_row_link` | `Devices_RowLink` | row | `N` |
| Link copied | Ссылка скопирована | Link copied | `devices_link_copied` | `Servers_LinkCopied` | transient, shared key | `N` |
| QR title | QR-код подписки | Subscription QR code | `devices_qr_title` | `Devices_QrTitle` | QR sheet | `N` |
| QR body | Отсканируйте в приложении на другом устройстве | Scan it in the app on the other device | `devices_qr_body` | `Devices_QrBody` | QR sheet | `N` |
| Add devices | Добавить устройства | Add devices | `devices_add_row` | `Devices_AddRow` | row and sheet title | `AP` |
| Price per device | %1$s за устройство | %1$s per device | `devices_add_per_device` | `Devices_AddPerDevice` | add-devices sheet | `N` |
| Estimate note | Точную сумму посчитаем при оплате | We'll calculate the exact amount at checkout | `devices_add_note` | `Devices_AddNote` | add-devices sheet | `N` |
| Go to payment | Перейти к оплате | Go to payment | `devices_add_pay` | `Devices_AddPay` | add-devices sheet | `N` |
...
| Count line | Подключено %1$d из %2$d | %1$d of %2$d connected | `devices_count_line` | `Devices_CountLine` | list header | `AP` |
| Count, unlimited | Подключено %1$d, без ограничений | %1$d connected, unlimited | `devices_count_line_unlimited` | `Devices_CountLineUnlimited` | list header | `N` |
...
| Platform and last seen | %1$s · активно %2$s | %1$s · active %2$s | `devices_platform_active` | `Devices_PlatformActive` | row subtitle (R-18) | `AP` |
| Unlink | Отвязать устройство | Unlink device | `devices_unlink` | `Devices_Unlink` | row action | `A` (R-3) |
| Unlink, short | Отвязать | Unlink | `devices_unlink_short` | `Devices_UnlinkShort` | confirm button | `A` |
| Unlink confirm title | Отвязать устройство? | Unlink this device? | `devices_unlink_title` | `Devices_UnlinkConfirm` | confirm dialog | `A` |
| Unlink confirm body | Устройство «%1$s» будет отвязано от подписки. | Device "%1$s" will be unlinked from your subscription. | `devices_unlink_body` | `Devices_UnlinkBody` | confirm dialog | `A` |
| Unlinked | Устройство отвязано | Device unlinked | `devices_unlinked` | `Devices_Unlinked` | transient with «Отменить» | `A` |
| Unlink failed | Не удалось отвязать устройство. Повторите попытку позже. | Couldn't unlink the device. Try again later. | `devices_unlink_failed` | `Devices_UnlinkFailed` | inline, with «Повторить» | `A` |

`A` on the six unlink strings: R-3. Android's set says «удалить», and its confirm body says
«отключено», which collides with the connection vocabulary. Deleted: `devices_hwid` / `Devices_Id`
(«ID: %1$s», R-18), `devices_delete_cd` (a screen-reader label that repeated the dialog title
instead of naming the button), and the whole `devices_diag_*` family - `devices_diag_http`
(«HTTP: %1$d»), `devices_diag_title` («Ответ сервера (диагностика)»), `devices_diag_failed` and
`devices_diag_empty`, both of which print the raw server response and ask the user for a screenshot
(C4).
...
| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Screen title | История платежей | Payment history | `history_title` | `Common_PaymentHistory` | sub-page | `=` |
...
| Unknown | Не определён | Unknown | `history_status_unknown` | `History_StatusUnknown` | row status chip | `N` |
| Kind: renewal | Продление | Renewal | `history_kind_renew` | `History_KindRenew` | row title | `AP` |
| Kind: purchase | Покупка подписки | Subscription purchase | `history_kind_purchase` | `History_KindPurchase` | row title | `N` |
| Kind: top-up | Пополнение баланса | Balance top-up | `history_kind_topup` | `History_KindTopUp` | row title | `=` |
| Kind: devices | Дополнительные устройства | Additional devices | `history_kind_devices` | `History_KindDevices` | row title | `=` |
| Kind: upgrade | Улучшение тарифа | Plan upgrade | `history_kind_upgrade` | `History_KindUpgrade` | row title | `N` |
| Kind: other | Платёж | Payment | `history_kind_other` | `History_KindOther` | row title | `N` |

**3.4.8 Sign-in methods**

| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Screen title | Способы входа | Sign-in methods | `linking_title` | `Account_LinkingTitle` | sub-page | `=` |
| Telegram | Telegram | Telegram | `linking_telegram` | `Linking_Telegram` | row title (brand, not translated) | `P+` |
| Email | Почта | Email | `linking_email` | `Linking_Email` | row title | `P` |
| Google | Google | Google | `linking_google` | `Linking_Google` | row title (brand) | `P+` |
| Linked | Привязан | Linked | `linking_linked` | `Account_Linked` | row value, masculine | `P` |
| Not linked, m. | Не привязан | Not linked | `linking_not_linked_m` | `Linking_NotLinkedM` | Telegram, Google rows | `N` |
| Not linked, f. | Не привязана | Not linked | `linking_not_linked_f` | `Linking_NotLinkedF` | Почта row | `N` |
| Link | Привязать | Link | `linking_action` | `Account_LinkAction` | row action | `=` |
| Linking via the site | Привязка через сайт | Linking happens on the website | `linking_via_site` | `Linking_ViaSite` | Google row subtitle | `N` |
| Telegram sheet title | Привязка Telegram | Link Telegram | `linking_tg_sheet_title` | `Linking_TgSheetTitle` | sheet | `N` |
| Telegram sheet body | Откройте бота и подтвердите привязку. | Open the bot and confirm the link. | `linking_tg_sheet_body` | `Linking_TgSheetBody` | sheet | `N` |
| Open the bot | Открыть бота | Open the bot | `linking_tg_open_bot` | `Account_OpenBot` | sheet | `=` |
| Link code | Код: %1$s | Code: %1$s | `linking_tg_code` | `Account_TgLinkCode` | sheet | `A+` |
| Waiting | Ждём подтверждения… | Waiting for confirmation… | `linking_tg_waiting` | `Account_TgLinkWaiting` | sheet (R-13) | `P` |
| Telegram linked | Telegram привязан | Telegram linked | `linking_tg_done` | `Linking_TgDone` | transient | `P+` |
| Email sheet title | Привязка почты | Link an email | `linking_email_sheet_title` | `Account_EmailLinkTitle` | sheet | `P` |
| Email sheet body | Пришлём ссылку для подтверждения на этот адрес. | We'll email a confirmation link to this address. | `linking_email_hint` | `Account_EmailLinkHint` | sheet | `A+` |
| Send | Отправить | Send | `linking_email_send` | `Account_Send` | sheet | `A+` |
| Email sent | Письмо отправлено. Проверьте почту. | Email sent. Check your inbox. | `linking_email_sent` | `Account_EmailSent` | transient | `AP` |

`P` on «Привязан»: the string exists on PC and is never rendered - the rows signal "linked" with a
check glyph alone, which `00-rules.md` 14.7 forbids (colour or icon is never the only signal).
`AP` on the sent confirmation: PC's «Письмо отправлено на {0}» repeats an address the user just
typed; the shorter form leaves room for the instruction.

### 3.5 Sign-in

Canonical spec: `14-auth.md` 11. The desktop's sign-in surface is being rebuilt against the same
state machine (PG-A12), so most PC rows are `P` or `P+`: the strings exist but under keys that
described a different layout.
...
| Gate body | Здесь будут подписка, устройства и платежи. | Your subscription, devices and payments live here. | `auth_gate_body` | `Login_Subtitle` | gate | `P` |
| Sign in with Telegram | Войти через Telegram | Sign in with Telegram | `auth_btn_telegram` | `Common_SignInTelegram` | gate primary (Android), demoted (PC, DV-1) | `=` |
| Sign in with email | Войти по почте | Sign in with email | `auth_btn_email` | `Login_ByEmail` | gate | `P+` |
| Waiting title | Ждём подтверждения в Telegram | Waiting for Telegram | `auth_awaiting_title` | `Login_WaitingConfirm` | gate, awaiting | `P` (R-13) |
| Waiting body | Подтвердите вход в открывшемся приложении | Confirm the sign-in in the app that opened | `auth_awaiting_body` | `Login_TelegramConfirmHint` | gate, awaiting | `P` |
| Open Telegram | Открыть Telegram | Open Telegram | `auth_open_telegram` | `Login_OpenTelegram` | gate, awaiting | `=` |
| Start over | Начать заново | Start over | `auth_restart` | `Login_StartOver` | gate, awaiting | `=` |
| Another method | Другой способ входа | Another way to sign in | `auth_other_method` | `Login_ChooseAnother` | gate, method sheet title | `=` |

`P` on the waiting pair: PG-A5 and PG-A6 in `14-auth.md` 18.3 - «Ожидаем» becomes «Ждём», and
`Login_TelegramConfirmHint` carries a dash construction («…и вернитесь сюда. Остальное сделаем
сами.») that 9.2 bans and that promises work the app does not narrate.

**Surface B - the email form**

| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Sign-in title | Вход по почте | Sign in with email | `auth_email_title` | `Login_TabSignIn` | form, sign-in mode | `P` |
| Register title | Регистрация | Register | `auth_register_title` | `Login_TabRegister` | form, register mode | `=` |
| Method: password | Пароль | Password | `auth_method_password` | `Login_MethodPassword` | method slot | `P+` |
| Method: link | Ссылка на почту | Emailed link | `auth_method_link` | `Login_MagicLink` | method slot | `P` |
| Email label | Электронная почта | Email | `auth_email_label` | `Login_Email` | field label (C7) | `=` |
| Email placeholder | name@example.com | name@example.com | `auth_email_placeholder` | `Login_EmailPlaceholder` | field watermark | `P+` |
| Password label | Пароль | Password | `auth_password_label` | `Login_Password` | field label | `=` |
| Password rule | Не менее 8 символов | At least 8 characters | `auth_password_hint_register` | `Login_PasswordHint` | helper under the field, register | `P` |
...
| Link method hint | Отправим ссылку для входа на этот адрес. Откройте её на этом телефоне. | We'll email you a sign-in link. Open it on this phone. | `auth_link_hint` | `Login_MagicHint` | helper, link method | `P+` |
| Submit: sign in | Войти | Sign in | `auth_btn_signin` | `Login_SubmitSignIn` | form CTA | `=` |
| Submit: send link | Отправить ссылку | Send the link | `auth_btn_send_link` | `Login_SendLink` | form CTA, link method | `P+` |
| Submit: register | Создать аккаунт | Create account | `auth_btn_register` | `Login_CreateAccount` | form CTA, register mode | `=` |
...
| Create an account | Создать аккаунт | Create an account | `auth_create_account` | `Login_CreateAccount` | mode switch | `=` |
| I already have one | У меня уже есть аккаунт | I already have an account | `auth_have_account` | `Login_HaveAccount` | mode switch, register | `P+` |
| Or | или | or | `auth_or` | `Login_Or` | divider | `A+` |

**2FA**

| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Code label | Код из приложения-аутентификатора | Code from your authenticator app | `auth_2fa_label` | `Login_EnterCode` | 2FA step (C7) | `P` |
| Code (a11y) | Код из приложения, 6 цифр | Authenticator code, 6 digits | `auth_2fa_a11y` | `Login_EnterCodeA11y` | OTP component | `P+` |
...
| Wrong code | Неверный код. Проверьте приложение-аутентификатор. | Wrong code. Check your authenticator app. | `auth_2fa_wrong` | `Login_CodeWrong` | field error | `P+` |
...
| Link sent body | Мы отправили ссылку на %1$s. Откройте её на этом телефоне. | We've sent a link to %1$s. Open it on this phone. | `auth_sent_magic_body` | `Login_MagicSentHint` | sent block | `P` |
| Verify title | Подтвердите почту | Confirm your email | `auth_sent_verify_title` | `Login_VerifyTitle` | sent block | `=` |
| Verify body | Мы отправили ссылку на %1$s. Откройте её, чтобы завершить регистрацию. | We've sent a link to %1$s. Open it to finish registering. | `auth_sent_verify_body` | `Login_VerifyHint` | sent block | `P` |
| Reset title | Письмо отправлено | Email sent | `auth_sent_reset_title` | `Login_ResetSentTitle` | sent block | `=` |
| Reset body | Если аккаунт с %1$s существует, мы отправили ссылку для сброса пароля. Задайте новый пароль и вернитесь ко входу. | If an account for %1$s exists, we've sent a password-reset link. Set a new password, then come back to sign in. | `auth_sent_reset_body` | `Login_ResetSentHint` | sent block | `P` |
...
`P` on the three bodies: PG-A7 - all three PC strings carry a dash construction that 9.2 bans.

**Surface C - the method sheet, and the website hand-off**

| Концепт | Русский | English | Android | PC | Экраны | Δ |
|---|---|---|---|---|---|---|
| Sheet title | Другой способ входа | Another way to sign in | `auth_methods_title` | `Login_ChooseAnother` | method sheet | `=` |
| Via the website | Через сайт | On the website | `auth_method_site` | `Login_ViaSite` | method sheet | `P+` |
| Via the website, sub | Откроем departament.site в браузере | We'll open departament.site in your browser | `auth_method_site_sub` | `Login_ViaSiteSub` | method sheet | `P+` |
...
| Via Google | Через Google | With Google | `auth_method_google` | `Login_ContinueGoogle` | method sheet, only when configured | `P` |
| Via Google, sub | Войдите аккаунтом Google | Sign in with your Google account | `auth_method_google_sub` | `Login_GoogleSub` | method sheet | `P+` |
| Hand-off field label | Код из браузера | Code from the browser | `auth_handoff_label` | `Login_HandoffLabel` | hand-off field (C7) | `P+` |
...
| Stage: subscription | Загружаем подписку | Loading your subscription | `auth_sync_stage_subscription` | `Account_SyncSubtitle` | hand-off | `P` |
| Stage: servers | Загружаем серверы | Loading servers | `auth_sync_stage_servers` | `Account_SyncStageServers` | hand-off | `P` |
| Sync failed title | Не удалось синхронизировать | Sync didn't finish | `auth_sync_error_title` | `Account_SyncErrorTitle` | hand-off error | `A+` |
| Sync failed hint | Проверьте подключение и повторите. | Check your connection and try again. | `auth_sync_error_hint` | `Account_SyncErrorHint` | hand-off error | `A+` |
| Retry | Повторить | Try again | `common_retry` | `Common_Retry` | hand-off error | `=` |
| Sign in again | Войти заново | Sign in again | `auth_sync_relogin` | `Account_SyncReLogin` | hand-off error | `A+` |
| Link Telegram title | Привязать Telegram | Link Telegram | `auth_link_tg_title` | `Account_RowLinkTelegram` | surface E, shared with the account row | `=` |
| Telegram linked | Telegram привязан | Telegram linked | `auth_link_tg_done` | `Linking_TgDone` | transient, shared | `=` |

**Deleted from the auth surface** (`14-auth.md` 11.7): `auth_tg_headline`, `auth_tg_desc`,
`auth_site_headline`, `auth_site_desc`, `auth_btn_site` («Войти через сайт» - the button posted
email and password, so the label lied), `auth_register_site`, `auth_awaiting`, `auth_fields_required`
(the CTA is disabled until the form is valid, so it can never fire), `auth_err_dialog_title`,
`auth_title`, `auth_sign_in_telegram` and `auth_sign_in_site` (duplicates of `auth_btn_*`).
...
|---|---|---|---|---|
| Настройки | Settings | `set_title` | `Nav_Settings` | `=` |
...
| Вместе | Both | `set_mode_both` | `Settings_ModeBoth` | `N` |
...
| Прокси по приложениям | Per-app proxy | `set_perapp` | `Settings_PerApp` | `=` |
| Кроме %1$d | All except %1$d | `set_perapp_except` | `Settings_PerAppExcept` | `AP` |
| Только %1$d | Only %1$d | `set_perapp_only` | `Settings_PerAppOnly` | `AP` |
| Не выбрано | Nothing selected | `set_perapp_none` | `Settings_PerAppNone` | `N` |
| Маршрутизация | Routing | `set_routing` | `Settings_Routing` | `=` |
| Обход локальной сети | Bypass the local network | `set_bypass_lan` | `Settings_BypassLan` | `=` |
| Прямой доступ к устройствам в локальной сети | Direct access to devices on your local network | `set_bypass_lan_hint` | `Settings_BypassLanHint` | `=` |
| Включить IPv6 в туннеле | Enable IPv6 in the tunnel | `set_ipv6_hint` | `Settings_Ipv6Hint` | `=` |
| Дополнительно | Advanced | `set_advanced` | `Settings_Advanced` | `P+` |
...
| Параметры фрагментации | Fragmentation options | `set_fragment_params` | `Settings_FragmentParams` | `N` |
...
| Выключено | Off | `set_sub_auto_off` | `Settings_SubAutoOff` | `N` |
...
| Проверка задержки | Latency testing | `set_latency` | `Settings_Latency` | `N` |
| Настройки провайдеров | Provider settings | `set_providers` | `Provider_Title` | `=` |
| Файлы ресурсов | Resource files | `set_assets` | `Settings_GeoFiles` | `=` |
...
| Системный | System | `set_language_system` | `Settings_LanguageSystem` | `=` |
...
| Открывать departament при входе в систему | Open departament when you sign in | `set_boot_hint` | `Settings_AutostartHint` | `A` |
| Масштаб интерфейса | Interface scale | - | `Settings_UiScale` | `P+` |
| Окно и горячие клавиши | Window and shortcuts | - | `Settings_Window` | `P+` |
| Данные и резервные копии | Data and backups | `set_data` | `Settings_Data` | `AP` |
| О приложении | About | `set_about` | `Settings_About` | `=` |
| Версия %1$s | Version %1$s | `set_about_version` | `About_VersionValue` | `=` |

**Notes.** `AP` on «Режим подключения»: Android's row is «Режим VPN» and PC's is «Режим»; neither
says what is being chosen. `A` on the Mux pair: R-2, plus PC's shorter hint, which does not restate
the title. `AP` on «Провайдеры» as the group header: R-4 replaces `12-settings.md`'s «Подписки»,
and the row inside the group keeps the name both platforms already ship, «Настройки провайдеров»
(Android `ps_title`, PC `Provider_Title`) - so a group and a row inside it no longer carry the same
word, which is `40-copy-inventory-android.md` F7.1's most common duplicate shape. `A` on «Запуск при
старте»: Android's current helper («Подключаться после перезагрузки устройства») promises a
connection the boot receiver does not make. `AP` on «Меньше движения»: Android has no key at all and
PC calls it «Облегчённый режим», which names a mode rather than an effect.
...
| Поиск по приложениям | Search apps | `set_perapp_search` | `PerApp_Search` | `AP` (R-9) |
| Добавить программу | Add a program | - | `PerApp_AddExe` | `P` |
| Выбрать все | Select all | `set_perapp_all` | `PerApp_SelectAll` | `N` |
| Снять выделение | Clear the selection | `set_perapp_none_action` | `PerApp_ClearSelection` | `N` |
...
| Из QR-кода | From a QR code | `set_routing_import_qr` | `Routing_ImportQr` | `N` |
| Стандартные наборы | The built-in sets | `set_routing_import_preset` | `Routing_ImportPreset` | `N` |
| Домены | Domains | `set_routing_sec_domains` | `Routing_SecDomains` | `N` |
...
| Удалит все наборы, включая свои | This deletes every set, including your own | `set_routing_reset_hint` | `Routing_ResetHint` | `N` |
| Название | Name | `set_rule_name` | `Rule_Name` | `N` |
...
| Домены | Domains | `set_rule_domains` | `Rule_Domains` | `N` |
| Один домен в строке. Поддерживаются geosite: | One domain per line. `geosite:` is supported | `set_rule_domains_hint` | `Rule_DomainsHint` | `N` |
| IP-адреса | IP addresses | `set_rule_ips` | `Rule_Ips` | `N` |
| Один адрес или диапазон в строке. Поддерживаются geoip: | One address or range per line. `geoip:` is supported | `set_rule_ips_hint` | `Rule_IpsHint` | `N` |
...
| Приложения | Apps | - | `Rule_Processes` | `P+` |
| Имена процессов, по одному в строке | Process names, one per line | - | `Rule_ProcessesHint` | `P+` |

`AP` on «%1$d правил»: an ungrammatical count at n=1 and n=2 on both platforms; section 4 makes it a
...
| По умолчанию | Default | `set_dns_default` | `Common_Default` | `=` |
| Свой | Custom | `set_dns_custom` | `Common_Custom` | `=` |
...
| Системный | System | `set_dns_system` | `Dns_System` | `N` |
...
| Как у провайдера | Provider order | `set_providers_sort_default` | `Provider_SortDefault` | `N` |
| По задержке | By latency | `set_providers_sort_ping` | `Provider_SortPing` | `N` |
| По имени | By name | `set_providers_sort_name` | `Provider_SortName` | `N` |
...
| %1$s МБ · обновлён %2$s | %1$s MB · updated %2$s | `set_assets_meta` | `Geo_SizeUpdated` | `=` |
...
| Из QR-кода | From a QR code | `set_assets_add_qr` | `Geo_AddQr` | `N` |
| Ссылка | Link | `set_assets_url_label` | `Geo_UrlLabel` | `N` |
| Базы обновлены | The databases are up to date | `set_assets_done` | `Geo_Done` | `P` |

`AP` on «Источник обновлений»: Android's `asset_geo_files_sources` reads «Источник геофайлов
(необязательно)» and PC's `TbSettingsGeoFilesSource` uses «источник» for a provider, which 9.3 bans;
here it genuinely is an update source, and the label says so without the parenthesis. `P` on «Базы
...
| Ядро | Core | `set_adv_sec_core` | `Adv_SecCore` | `N` |
| Ядро | Core | - | `Adv_Core` | `P+` |
| Уровень журнала | Log level | `set_adv_loglevel` | `Adv_LogLevel` | `N` |
| Никакой | None | `set_adv_loglevel_none` | `Adv_LogNone` | `N` |
| Ошибки | Errors | `set_adv_loglevel_error` | `Adv_LogError` | `N` |
...
| Локальный прокси | Local proxy | `set_adv_localproxy` | `Settings_LocalProxy` | `=` |
...
`A` on «Постоянный VPN»: today the row lives in the hub's Подключение group and fires a `Toast`
before launching the system intent; the toast is deleted (1.4.8) and the helper carries the same
fact permanently.
...
| Пароль | Password | `set_lp_pass` | `Lp_Password` | `=` |
...
| Адрес действует, пока устройство в этой сети | The address works while this device stays on this network | `set_lp_endpoint_hint` | `Lp_EndpointHint` | `N` |
...
| Все настройки, подписки и серверы сохраняются в один файл. | Every setting, provider and server is saved into a single file. | `set_data_intro` | `Backup_Intro` | `P` |
...
| Облачная копия | Cloud backup | `set_data_cloud` | `Backup_Cloud` | `A` |
| Устройства | Devices | `set_data_sec_devices` | - | `A` |
...
| Серверы и подписки останутся | Your servers and providers stay | `set_data_reset_hint` | `Backup_ResetHint` | `N` |
...
| Облачная копия | Cloud backup | `set_webdav_title` | `Webdav_Title` | `A` |
| Адрес сервера | Server address | `set_webdav_url` | `Webdav_Url` | `A` |
...
`AP` on the three backup verbs: Android ships «Резервирование конфигурации» / «Восстановление
...
has four words for two actions. `A` on the WebDAV block: those rows exist only on Android today and
are English (`title_webdav_url`, `title_webdav_pass`), so they are rewritten rather than translated.

**3.6.12 `settings/window` - «Окно и горячие клавиши» (PC only)**

| Русский | English | Android | PC | Δ |
|---|---|---|---|---|
| Окно и горячие клавиши | Window and shortcuts | - | `Window_Title` | `P+` |
| Окно | Window | - | `Window_SecWindow` | `P+` |
| Сворачивать в трей при закрытии | Minimise to the tray when closed | - | `Window_HideToTray` | `P+` |
...
tooltip also carries two em-dashes and a U+2212 minus sign.
...
| Поддержка | Support | `set_about_sec_support` | `About_SecSupport` | `N` |
...
| Проверить обновления | Check for updates | - | `About_CheckUpdate` | `P` |
...
| Правовое | Legal | `set_about_sec_legal` | `About_SecLegal` | `N` |
...
| ОС: %1$s\nАрхитектура: %2$s\n.NET: %3$s | OS: %1$s\nArchitecture: %2$s\n.NET: %3$s | `set_about_system_info` | `About_SystemInfo` | `A+` |
...
| Запустить туннель | Start the tunnel | `scheme_label_connect` | `UrlSchemes_StartTunnel` | `A` |
| Открыть приложение | Open the app | `scheme_label_open` | `UrlSchemes_OpenApp` | `=` |
| Отключиться | Disconnect | `scheme_label_disconnect` | `UrlSchemes_Stop` | `=` |
...
| Ошибки | Errors | `set_log_errors` | `Log_Errors` | `N` |
| Скопировать всё | Copy everything | `set_log_copy` | `Log_CopyAll` | `N` |
| Очистить | Clear | `set_log_clear` | `Log_Clear` | `N` |

`P` on `UrlSchemes_Close`: the string exists and is correct, and the page labels **both**
`depv://disconnect` and `depv://close` with `UrlSchemes_Stop` («Отключиться»)
(`UrlSchemesPage.axaml.cs:33-34`). One of the two rows is therefore a lie about what the scheme
does. `AP` on the licences dialog: `AboutActivity.kt:31,33` opens it with an English hardcoded title
(«Open source licenses») and an «OK» button, on a screen whose own row is Russian (R-15).
